...
Token Tokenizer::getToken() {
    ...
    if (c == '<') { // A possible open tag.
        ...
        //tempTag = getTokenTagName(c);
        std::string tagName = "";
        if (inputStream.peek() == '/') { // probably a closed tag. Something like </table
            inputStream.get(c);
            //tagName += c;
            charPosition++;
            ...
            while(isalnum(inputStream.peek()))
            {
                ...
                charPosition++;
                tagName += c;
            }  // probably an open tag. something like <table
            token.makeOpenTag(tagName);
        }else{
            // this is a random open angle breackt. Put it in a token and return it.
            tagName = "";
            token.makeOpenAngleBracket();
        } //} else if( c == '/' ) { // must be a stand-alone close tag. }

        // after filling the tag we then need to idenfity this token as openAngleBrace Token
        token.setOpenAngleBracket();
        ...
        //std::cout << " " << c << ": " << this->charPosition << std::endl;
        charPosition++;
        token.makeCloseStandAloneTag();
        // sets token so that it is identifyable as a close angle brace;
        //token.setCloseAngleBracket();
        token.endOfFile() = false;
        return token;
    }
    else if (c == '>') {
